...
import argparse
import numpy as np
# SB3 imports are done lazily inside from-policy flow to avoid dependency issues
import yaml

# Import our custom env and wrappers
import energy_net.env.register_envs
from energy_net.alternating_wrappers_model_free import make_pcs_env
...
def generate_pattern_actions(pattern_type, sequence_length, cycle_length=48, seed=42, pattern_config=None):
    ...
    np.random.seed(seed)
    # noam noya comment : noise
    noise = np.random.normal(0, 0, size=sequence_length)
    ...
    if pattern_type == "charge_discharge_cycle":
        ...
        pattern = np.concatenate([
            ...
            # np.ones(cycle_length // 2) * 0,  # Charge (negative values in our convention)
            # np.ones(cycle_length // 2) * 0    # Discharge (positive values)
        ])
        ...
        pattern = np.concatenate([
            np.ones(third) * 0.5,              # Night: charge heavily (low prices)
            np.ones(third) * -0.2,               # Day peak: discharge heavily (high prices)
            np.ones(third) * -0.1                # Evening: light discharge (moderate prices)
        ])
