...
class EnergyNetController:
    ...
    def __init__(
        ...
        iso_reward_fn: Optional[BaseReward] = None,
    ):
        """
        ...
        self.use_dispatch_action = self.iso_config.get('dispatch', {}).get('use_dispatch_action', True)
        ...
        # Initialize ISO reward function (allow injection)
        self.iso_reward_function = iso_reward_fn if iso_reward_fn is not None else ISOReward()
    ...
    def _init_iso_components(self, dispatch_config: Optional[Dict[str, Any]]):
        ...
        if dispatch_config:
            self.use_dispatch_action = dispatch_config.get('use_dispatch_action', 
                                       dispatch_config_from_file.get('use_dispatch_action', True))
        else:
            self.use_dispatch_action = dispatch_config_from_file.get('use_dispatch_action', True)
