try:
    ...
    # Minimal stub so this module can be imported without SB3 installed
...
def plot_episode_detail(episode_data, episode_num, output_dir):
    """
    Fixed version of plot_episode_detail that can handle both flat and nested data structures.
    """
    # Extract data
    steps = range(len(episode_data))
    
    # Helper function to safely extract data from either flat or nested structure
    def safe_extract(data_list, key_paths, default=0.0):
        """
        Safely extract data from a list of dictionaries, trying multiple key paths.
        """
        result = []
        for step_data in data_list:
            ...
            for key_path in key_paths:
                if '.' in key_path:
                    # Handle nested paths like 'info.iso.predicted_demands'
                    keys = key_path.split('.')
                    current = step_data
                    found = True
                    for key in keys:
                    ...
                    if found:
                        if isinstance(current, list) and len(current) > 0:
                            value = current[0]  # Take first element if it's a list
                        else:
                            value = current
                        break
                else:
                    # Handle flat paths
                    if key_path in step_data:
                        value = step_data[key_path]
                        break
            result.append(value)
        return result
    
    # Extract data using the safe extraction function
    predicted_demands = safe_extract(episode_data, 
                                   ['predicted_demand', 'info.iso.predicted_demands'])
    realized_demands = safe_extract(episode_data, 
                                  ['realized_demand', 'info.iso.realized_demands'])
    pcs_demands = safe_extract(episode_data, 
                              ['pcs_demand', 'info.iso.pcs_demands'])
    net_demands = safe_extract(episode_data, 
                              ['net_demand', 'info.iso.net_demands'])
    buy_prices = safe_extract(episode_data, 
                             ['iso_buy_price', 'info.iso.buy_prices'])
    sell_prices = safe_extract(episode_data, 
                              ['iso_sell_price', 'info.iso.sell_prices'])
    
    # Get PCS metrics
    battery_levels = safe_extract(episode_data, 
                                 ['battery_level', 'info.pcs.battery_levels'])
    energy_exchanges = safe_extract(episode_data, 
                                   ['net_exchange', 'info.pcs.energy_exchanges'])
    
    # Get dispatch and costs
    dispatch_costs = safe_extract(episode_data, 
                                 ['dispatch_cost', 'info.iso.dispatch_costs'])
    reserve_costs = safe_extract(episode_data, 
                                ['reserve_cost', 'info.iso.reserve_costs'])
    dispatch = safe_extract(episode_data, 
                           ['dispatch', 'info.shared.dispatch'])
