"""
Environment wrappers for alternating training of ISO and PCS agents (model-free).
...
# Monitor import will be done locally to avoid compatibility issues

# Set up basic logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("alternating_wrappers_model_free")
...
class ISOEnvWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # --- Extract rewards with hard failures (no silent fallbacks) ---
        if isinstance(rewards, dict):
            if 'iso' not in rewards or 'pcs' not in rewards:
                raise RuntimeError(f"Env returned rewards without required keys: {rewards!r}")
            iso_reward = rewards['iso']
            pcs_reward = rewards['pcs']
        elif isinstance(rewards, (list, tuple, np.ndarray)):
            if len(rewards) < 2:
                raise RuntimeError(f"Env returned non-dict rewards with insufficient length: {rewards!r}")
            iso_reward = rewards[0]
            pcs_reward = rewards[1]
        else:
            raise RuntimeError(f"Unsupported rewards type {type(rewards).__name__}: {rewards!r}")

        # Sanity check: rewards must be finite numbers
        if not (np.isfinite(iso_reward) and np.isfinite(pcs_reward)):
            raise RuntimeError(f"Non-finite reward detected: iso={iso_reward}, pcs={pcs_reward}")

        # Inject rewards and demand values into info for Monitor
        if info is None:
            info = {}
        info['iso_reward'] = float(iso_reward)
        info['pcs_reward'] = float(pcs_reward)
        
        # Extract demand values from the environment's info for Monitor logging
        if 'predicted_demand' in info:
            info['predicted_demand'] = float(info['predicted_demand'])
        if 'realized_demand' in info:
            info['realized_demand'] = float(info['realized_demand'])
        if 'net_exchange' in info:
            info['net_exchange'] = float(info['net_exchange'])
        if 'battery_level' in info:
            info['battery_level'] = float(info['battery_level'])
...
def make_iso_env(
    ...
    try:
        print(
            "[Factory] make_iso_env called:",
            dict(cost_type=cost_type, pricing_policy=pricing_policy, demand_pattern=demand_pattern,
                 seed=seed, eval_mode=eval_mode, use_dispatch_action=use_dispatch_action,
                 norm_path=norm_path),
            flush=True,
        )
    except Exception:
    ...
    # Create the vectorized environment (lazy SB3 import)
...
def make_pcs_env(
    ...
    Args:
        ...
        demand pattern: Pattern of demand
        seed: Random seed
        log_dir: Directory for logs
        model_dir: Directory for saved models
        plot_dir: Directory for plots
    ...
    # Add monitoring wrapper (lazy SB3 import)
    from stable_baselines3.common.monitor import Monitor
    ...
    # Create vectorized environment (lazy SB3 import)
    from stable_baselines3.common.vec_env import DummyVecEnv, VecNormalize
...
def make_iso_env_single(
    ...
    """
    Create a non-vectorized ISO training environment without SB3 wrappers.
    Returns a raw gym.Env (ISOEnvWrapper or PreDefinedPCSWrapper).
    """
    try:
        print(
            "[Factory] make_iso_env_single called:",
            dict(cost_type=cost_type, pricing_policy=pricing_policy, demand_pattern=demand_pattern,
                 seed=seed, eval_mode=eval_mode, use_dispatch_action=use_dispatch_action),
            flush=True,
        )
    ...
    # Add monitor wrapper for logging (same as make_iso_env)
    # Try to import Monitor with better error handling
    try:
        # Try importing Monitor from stable_baselines3
        from stable_baselines3.common.monitor import Monitor
        
        # Ensure the monitor directory exists
        monitor_dir = f"{log_dir}/iso/monitor"
        os.makedirs(monitor_dir, exist_ok=True)
        
        env = Monitor(
            env,
            f"{monitor_dir}/{pricing_policy.value}_{cost_type.value}_{int(steps_per_iteration)}.csv",
            info_keywords=("iso_reward", "pcs_reward", "net_exchange", "battery_level", "predicted_demand", "realized_demand")
        )
        print(f"Successfully added Monitor wrapper for logging to {monitor_dir}/")
        
        # Force the Monitor wrapper to log data by ensuring it's properly initialized
        if hasattr(env, 'file_handler') and env.file_handler:
            print(f"Monitor file handler initialized: {env.file_handler.name}")
        else:
            print("Warning: Monitor file handler not properly initialized")
            
    except ImportError as e:
        print(f"Warning: Could not import Monitor wrapper due to compatibility issues: {e}")
        print("Monitor logging will not be available - this may cause plotting issues")
    except Exception as e:
        print(f"Warning: Could not add Monitor wrapper: {e}")
        print("Monitor logging will not be available")
